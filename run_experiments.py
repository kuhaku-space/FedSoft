...
from solvers import Cifar10CNNClientSolver, ServerSolver
from utils import init_exp, logger
from validators import ValidatorClassification, ValidatorConfig

SEEDS = [0, 1, 2, 3, 4]
GPU_MEMORY_FRACTION = 1.0  # 使用するVRAMの割合 (0.0〜1.0)
...
def set_gpu_memory_limit(fraction: float):
    if torch.cuda.is_available():
        ...
        tqdm.write(
            f"[GPU] Memory limit: {fraction * 100:.0f}% of {total_mb:.0f} MB "
...
def run_experiment(exp_id: str, seed: int):
    ...
    client_vec, test_ds_vec, preparation_dict = (
        create_clients_cifar_and_cifar_rotation_90(client_solver)
    )
